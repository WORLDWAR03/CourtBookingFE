...
function UpComingBookings({data}) {
  ...
  return (
    <div className=' mx-auto max-w-md bg:center gap-4 px-5  py-8 justify-center grid  md:grid-cols-1 place-items-center align-middle cursor-pointer   md:max-w-6xl bg-[#fff' onClick={cancelBooking}>
  ...
  <div  className=" align-center sm:max-w-sm md:max-w-xl md:max-h-[32rem] w-full lg:max-w-full cursor-pointer lg:flex rounded-lg bg-[#d8f3dc]  shadow-lg overflow-hidden" >
    ...
    <div  className="border-r lg:min-w-[400px] border-b border-l border-[#95d5b2] lg:border-l-0 lg:border-t-0 lg:border-r-0 lg:border-b-0  rounded-b lg:rounded-b-none lg:rounded-r px-4 flex flex-col justify-between leading-normal bg-[#d8f3dc]">
      <div  className="mb-8 pt-2 ">
        ...
        <h3 className='font-bold text-xl mt-2'>{data.courts.businessName}</h3>
        <div  className="text-[#00424F] font-bold text-xl mb-2">{data.businessName}</div>
        <p>location: {data.courts.location}</p>
        <p>VenueType:{data.venueType} ({data.courts.feature})</p>
